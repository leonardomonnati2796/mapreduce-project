# Script per eseguire i test copiando i file necessari
...
Copy-Item "test/loadbalancer_test_integration.go" -Destination "$testDir/" -Force
...
go test -v loadbalancer_test_integration.go loadbalancer.go health.go config.go rpc.go
